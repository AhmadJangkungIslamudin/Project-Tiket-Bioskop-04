...
def transaksi(jenis_transaksi):
    if jenis_transaksi =='1':
        ...
        reschedule = pd.read_csv('riwayat pembelian.csv')
        with open("riwayat pembelian.csv", "w", newline="") as reschedule:
                reader = csv.reader(reschedule)
                writer = csv.writer(reschedule)

                for row in reader:
                    if row[2] == 'A':  # Contoh kondisi untuk mengubah data tanggal dan jam
                        # Modifikasi data tanggal dan jam di sini sesuai kebutuhan
                        tanggal = datetime.strptime(row[0], '%Y-%m-%d')  # Ubah string menjadi objek datetime
                        jam = datetime.strptime(row[1], '%H:%M')  # Ubah string menjadi objek datetime

                        # Ubah data tanggal dan jam
                        tanggal_baru = tanggal.strftime('%d-%m-%Y')  # Format tanggal baru
                        jam_baru = jam.strftime('%I:%M %p')  # Format jam baru (12 jam)

                        # Masukkan data yang sudah diubah ke dalam row
                        row[0] = tanggal_baru
                        row[1] = jam_baru
